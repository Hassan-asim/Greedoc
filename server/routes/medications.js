...
router.post(
  ...
  [
    ...
    body("dosage.value")
      .isNumeric()
      .withMessage("Dosage value must be a number"),
    body("dosage.unit")
      .isIn([
        "mg",
        "g",
...
router.put(
  ...
  [
    ...
    body("discontinuedReason")
      .optional()
      .trim()
      .notEmpty()
      .withMessage("Discontinued reason cannot be empty"),
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
  ...
  try {
    ...
    const overallAdherence =
      medications.length > 0
        ? medications.reduce(
            (sum, med) => sum + med.adherence.adherenceRate,
            0
          ) / medications.length
        : 100;
